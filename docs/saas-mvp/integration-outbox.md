...
`lead.stage_changed` persiste `OutboxEvent` na mesma transação que move o Lead. O worker reclama a linha, Redis acorda o dispatch e, quando existe `Integration` com `provider=WEBHOOK` e `status=ENABLED`, o worker faz `POST` JSON para a URL configurada (com proteção SSRF e DNS pinning).
