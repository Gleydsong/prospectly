...
export function OpportunityCandidateModal({
  ...
  return (
    <Modal open={open} onClose={onClose} title={candidate.company.companyName} className="max-w-3xl">
      <div className="space-y-6">
        ...
        <section>
          ...
          <div className="space-y-2">
            {candidate.signals.map((signal) => {
              const Icon = signal.value === 'TRUE' ? CheckCircle2 : signal.value === 'FALSE' ? XCircle : CircleHelp;
              return (
                <div key={signal.type} className="flex gap-3 rounded-control border border-[color:var(--border)] p-3">
        ...
        {explanation ? (
          <Alert tone="info" title="Leitura comercial assistida">
            ...
            <p className="mt-2"><strong>Por que pode ser oportunidade:</strong> {explanation.whyOpportunity}</p>
            <p className="mt-2"><strong>Oferta sugerida:</strong> {explanation.recommendedOffer}</p>
            <p className="mt-2"><strong>Abordagem:</strong> {explanation.commercialAngle}</p>
          </Alert>
        ) : null}

        <Alert tone="warning">
          Sinais negativos são hipóteses de prospecção, não fatos sobre a necessidade da empresa. Valide antes de contactar.
        </Alert>
