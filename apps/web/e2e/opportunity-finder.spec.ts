...
test.describe('AI Opportunity Finder happy path', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/v1/**', async (route) => {
      ...
      if (path.endsWith('/opportunity-finder/runs/00000000-0000-4000-8000-000000000001'))
        return json({
          id: '00000000-0000-4000-8000-000000000001',
          status: 'COMPLETED',
          service: 'Criação de sites',
          ...
          profile: {
            ...
            targetCustomer: ['lojas de roupas'],
            categories: ['clothes'],
          ...
          data: [
            {
              ...
              confidenceScore: 72,
              dataCompleteness: 68,
              rankingCategory: 'EXCELLENT',
              importedLeadId: null,
            },
          ],
          total: 1,
        });
      if (path.endsWith('/save-lead') && request.method() === 'POST')
        return json({ status: 'CREATED', leadId: 'lead-1' });
      return json({
        creditBalance: 10,
  ...
  test('opens from Tools, ranks evidence and saves a reviewed lead', async ({ page }) => {
    ...
    await page
      .getByRole('link', { name: /Localizador de Oportunidades com IA/i })
      .first()
      .click();
    await expect(page).toHaveURL(/\/tools\/opportunity-finder$/);
    await expect(
      page.getByRole('heading', { level: 1, name: 'Localizador de Oportunidades com IA' }),
    ).toBeVisible();
