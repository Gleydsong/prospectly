export type WaitlistEmailLocale = 'pt' | 'en';
...
function landingBaseUrl(): string {
  const raw = process.env.NEXT_PUBLIC_LANDING_URL?.trim() || 'https://prospectlyonboard.com';
  const base = raw.replace(/\/$/, '');
  // Recipients cannot load localhost assets from their inbox.
