...
function V2Brand() {
  return <span className="landing-v2-brand" aria-label="Prospectly">prospectly<span>.</span></span>;
}

export function V2Header({ locale, page = 'home' }: { locale: Locale; page?: 'home' | 'how' }) {
  const [open, setOpen] = useState(false);
  const enterUrl = enterExplainerUrl(locale);
  const howUrl = '/v2/como-funciona';
  const benefitsUrl = page === 'how' ? '/v2#beneficios' : '#beneficios';
  const audienceUrl = page === 'how' ? '/v2#para-quem' : '#para-quem';
  const faqUrl = page === 'how' ? '/v2#faq' : '#faq';
  ...
  return (
    <section className="landing-v2-hero" aria-labelledby="landing-v2-hero-title">
      ...
      <div className="landing-v2-shell landing-v2-hero-content">
        ...
        <h1 id="landing-v2-hero-title">Encontre as empresas certas<br />para o seu próximo cliente.</h1>
...
export function V2Footer({ page = 'home' }: { page?: 'home' | 'how' }) {
  const homePrefix = page === 'how' ? '/v2' : '';
  return <footer className="landing-v2-footer"><div className="landing-v2-shell landing-v2-footer-grid"><div><V2Brand /><p>Empresas certas. Próximos passos claros.</p></div><div><strong>Produto</strong><Link href="/v2/como-funciona">Como funciona</Link><a href={`${homePrefix}#beneficios`}>Benefícios</a><a href={`${homePrefix}#faq`}>Perguntas frequentes</a></div><div><strong>Legal</strong><Link href="/terms">Termos de uso</Link><Link href="/privacy">Política de privacidade</Link><a href={`mailto:${TEAM_EMAIL}`}>Fale com a gente</a></div></div><div className="landing-v2-shell landing-v2-footer-bottom"><span>© {new Date().getFullYear()} Prospectly</span><span>V2 · Prospecção B2B com mais clareza.</span></div></footer>;
}
