...
export function IntentLanding({ content }: { content: IntentPageContent }) {
  return (
    <article className="bg-[color:var(--bg-sunken)]">
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <BentoSurface className="px-6 py-10 sm:px-10 sm:py-12">
          <Reveal>
            <h1 className="relative text-balance text-3xl font-semibold tracking-tight text-[color:var(--bento-ink)] md:text-4xl">
              {content.h1}
            </h1>
            <p className="relative mt-5 text-lg leading-relaxed text-[color:var(--bento-ink-muted)]">
              {content.lead}
            </p>
          </Reveal>
          ...
          <div className="relative mt-12 space-y-10">
            {content.sections.map((section) => (
              <Reveal key={section.heading}>
                <section>
                  <h2 className="text-xl font-semibold tracking-tight text-[color:var(--bento-ink)] md:text-2xl">
                    {section.heading}
                  </h2>
                  <p className="mt-3 text-base leading-relaxed text-[color:var(--bento-ink-muted)]">
                    {section.body}
                  </p>
                </section>
              </Reveal>
          ...
          <Reveal className="relative mt-14 flex flex-wrap gap-3">
