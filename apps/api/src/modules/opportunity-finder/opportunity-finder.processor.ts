  ...
  async process(job: Job<ProcessOpportunityRunJobData>): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'NO_COMPANIES_FOUND') {
        await this.service.recordFailure(job.data.runId, 'NO_COMPANIES_FOUND');
      ...
      if (finalAttempt) await this.service.recordFailure(job.data.runId, error instanceof Error ? error.message : 'PROCESSING_FAILED');
      throw error;
    }
  }
}
