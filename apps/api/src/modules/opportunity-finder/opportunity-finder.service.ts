...
export class OpportunityFinderService {
  ...
  async create(
    ...
    const category = resolution.status === 'RESOLVED' ? resolution.category : undefined;
    if (category) {
  ...
  async processRun(runId: string): Promise<void> {
    ...
    const lockedCategories = category ? [category] : [];
    ...
    const providerIds = this.providers.list()
      ...
      .filter((provider) => category || provider.id === 'GOOGLE_PLACES')
