...
describe('AuthService', () => {
  ...
  it('refresh rejects legacy null organizationId when user has multiple orgs', async () => {
    const jwt = makeJwt();
    (jwt.verifyAsync as jest.Mock).mockResolvedValue({ sub: 'u1', jti: 'jti-1' });
    const prisma = makePrisma();
    prisma.refreshToken.findUnique = jest.fn().mockResolvedValue({
    ...
    (prisma.organizationMember as unknown as { findMany: jest.Mock }).findMany = jest
      .fn()
      .mockResolvedValue([
